...
export class OAuthManager {
  ...
  private async fetchUserInfo(
    provider: OAuthProviderType,
    accessToken: string
  ): Promise<{ id: string; username?: string; name?: string; email?: string; avatar?: string }> {
  ...
  ): { id: string; username?: string; name?: string; email?: string; avatar?: string } {
    switch (provider) {
      case 'google':
        return {
          ...
          email: data.email_verified ? String(data.email || '') : undefined,
      ...
      case 'discord':
        return {
          ...
          email: data.verified ? String(data.email || '') : undefined,
          avatar: data.avatar
      ...
      default:
        return {
          id: String(data.id || ''),
          username: String(data.username || data.login || ''),
          name: String(data.name || ''),
          email: String(data.email || ''),
          avatar: String(data.avatar || data.avatar_url || ''),
        };
    }
  }
  ...
  private generateUserId(provider: OAuthProviderType, providerUserId: string): string {
    return `user_${crypto.createHash('sha256').update(`${provider}:${providerUserId}`).digest('hex').substring(0, 24)}`;
  }
