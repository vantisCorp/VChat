...
export class RTCPPacketHandler {
  ...
  static parse(
    data: Uint8Array
  ): (RTCPSenderReport | RTCPReceiverReport | RTCPSDES | RTCPBye | RTCPFeedback)[] {
  ...
  private static parseReportBlock(data: Uint8Array, offset: number): RTCPReportBlock {
    return {
      ...
      cumulativePacketsLost:
        ((data[offset + 5] << 16) | (data[offset + 6] << 8) | data[offset + 7]) & 0xffffff,
   ...
   */
  static buildReceiverReport(ssrc: number, reportBlocks: RTCPReportBlock[] = []): Uint8Array {
